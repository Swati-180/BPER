...
const saveDraft = async (req, res) => {
  try {
    ...
    if (!department || !month || !year) {
      return res.status(400).json({ message: 'department, month and year are required.' });
    }
...
const updateSubmissionStatus = async (req, res) => {
  try {
    ...
    const allowedStatuses = ['under_review', 'approved', 'returned_for_revision', 'submitted'];

    if (!allowedStatuses.includes(status)) {
      return res.status(400).json({ message: 'Invalid status value.' });
    }

    const submission = await WdtSubmission.findById(req.params.id);
    if (!submission) return res.status(404).json({ message: 'Submission not found.' });
    ...
    if (status === 'returned_for_revision') {
      submission.revisionNote = comment || submission.revisionNote || '';
    }
