...
export function SixBySixScoring() {
  ...
  const loadActivities = useCallback(async () => {
    try {
      ...
      if (!deptId) {
        const me = await apiGet<{ department?: { _id?: string } | string }>("/auth/me");
        deptId = typeof me.department === "string" ? me.department : me.department?._id || "";
      ...
      const activities = await apiGet<Array<{ _id: string; name: string }>>(`/eper/activities/flat/${deptId}`);
      const mapped: ActivityRow[] = activities.map((activity, idx) => ({
        id: idx + 1,
      ...
      setError(err?.response?.data?.message || "Unable to load activities for scoring.");
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    await Promise.all(
      rows
        ...
        .map((row) => apiPost("/eper/sixbysix/score", {
          activityId: row.activityId,
          departmentId,
          parameters: {
            multipleLocns: row.multipleLocns || "M",
            routine: row.routine || "M",
            volumes: row.volumes || "M",
            manpower: row.manpower || "M",
            sops: row.sops || "M",
            erpTechnology: row.erpTechnology || "M",
            sensitivity: row.sensitivity || "M",
            criticality: row.criticality || "M",
            controls: row.controls || "M",
            proximity: row.proximity || "M",
            regulatory: row.regulatory || "M",
            skill: row.skill || "M",
          },
  ...
  if (error) {
    return <ErrorFallbackState title="6x6 scoring unavailable" message={error} />;
  }

  if (rows.length === 0) {
    return <EmptyState title="No activities to score" message="No activity records are available for 6x6 evaluation." />;
  }
