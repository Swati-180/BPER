...
const NAV_ITEMS = [
  { key: "eperDashboard", label: "ePER Dashboard", icon: LayoutDashboard },
  { key: "wizard", label: "WDT Entry", icon: ClipboardList },
  { key: "wdtReview", label: "WDT Review", icon: Target },
  { key: "sixbySixScoring", label: "6x6 Scoring", icon: BarChart2 },
  { key: "fitmentScoring", label: "Fitment Scoring", icon: LineChart },
  { key: "deepReports", label: "Deep Reports", icon: BookOpen },
];
  ...
  return (
    <div className="w-64 bg-corporateBlue-dark min-h-screen flex flex-col fixed left-0 top-0 text-slate-300 z-50">
      ...
      <nav className="flex-1 py-4 overflow-y-auto">
        ...
        <ul className="space-y-0.5">
          {ADMIN_ITEMS.map(({ key, label, icon: Icon }) => (
            <li key={key}>
              <button
                onClick={() => setActivePage(key)}
                className={`w-full flex items-center gap-3 px-5 py-2.5 transition-all duration-150 text-sm group ${
